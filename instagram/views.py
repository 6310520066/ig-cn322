...
def add_comment(request,post_id):
    ...
    if current_user != post.user.user:
        recipient_user = post.user.user
        sender_user = current_user
        NotificationComment.objects.create(recipient=recipient_user, sender=sender_user, post=post, comment=comment)
...
def like_post(request, post_id):
    ...
    if post.likes.filter(id=user.id).exists():
        ...
        post.likes.add(user)
        is_liked = True
        recipient = post.user.user # retrieve the User instance associated with the Profile object
        if user != recipient:
            Notification.objects.create(recipient=recipient, sender=user, post=post)
...
def follow(request,id):
    if request.method == 'GET':
        ...
        notifollow = NotificationFollow.objects.create(
            recipient=user_follow,
            sender=request.user,
            follow=True,
        )
...
def notifications(request):
    ...
    noticomments = request.user.noticomment.all()
    notifollows = request.user.notifollow.all()
    return render(request, 'instagram/notifications.html', {'notifications': notifications, 'noticomments': noticomments, 'notifollows': notifollows})


def clear_notifications(request):
    # Delete all notifications for the current user
    Notification.objects.filter(recipient=request.user).delete()
    NotificationComment.objects.filter(recipient=request.user).delete()
    NotificationFollow.objects.filter(recipient=request.user).delete()
